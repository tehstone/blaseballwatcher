...
class BetAdvice(commands.Cog):
    ...
    async def daily_message(self, season, day):
        file_path = os.path.join('data', 'season_sim', 'results', f"s{season}_d{day}_sim_results.json")
        if int(day) < 99 and os.path.exists(file_path):
        ...
        if day+1 == 28:
            message += "\n\n**Earlsiesta begins after these games, make sure to bet before they end!**"
